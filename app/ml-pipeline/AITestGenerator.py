...
class AITestGenerator:
    ...
    def generate_test(self, users_text: str) -> AITest:
        ...
        try:
            response = httpx.post(url     = REQUEST_URL,
        ...
        if response.status_code != 200:
            raise httpx.HTTPStatusError(f"Incorrect POST request with text: {response.text}")
        

        json_llm_response = json.loads(response.text)["result"]["alternatives"][0]["message"]["text"]
        llm_answer = json.loads(json_llm_response[7:len(json_llm_response)-3])["test"]      # Slicing removes compulsory JSON formating
...
if __name__ == "__main__":
    ...
    print(ai.generate_test("Кириллица — одна из самых известных и широко используемых письменностей в мире, особенно в странах Восточной Европы и на Балкана"))
